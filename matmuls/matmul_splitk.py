...
def matmul_split_k_kernel(
    ...
    for kiter in range(0, tl.cdiv(k, BLOCK_SIZE_K * SPLIT_K)):
        kk = kiter * BLOCK_SIZE_K * SPLIT_K
        mask_k = offs_k + kk < k
        a_frag = tl.load(a_ptr, mask=a_mask_m & mask_k[None, :], other=0)
        b_frag = tl.load(b_ptr, mask=mask_k[:, None] & b_mask_n, other=0)
        acc = tl.dot(a_frag, b_frag, acc, input_precision="ieee")

        a_ptr += BLOCK_SIZE_K * a_stride_k * SPLIT_K
        b_ptr += BLOCK_SIZE_K * b_stride_k * SPLIT_K
